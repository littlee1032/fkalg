...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase().filter(_.isLetter).groupBy(x => x).mapValues(_.size).toList.sorted
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.foldLeft(x.toMap)((map, ele) => {
      val freq = map(ele._1) - ele._2
      if (freq > 0) map.updated(ele._1, freq)
      else map - ele._1
    }).toList.sorted
  }
